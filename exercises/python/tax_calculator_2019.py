...
class TaxCalculator2019:
    ...
    def calculate_tax(self, vehicle):
        if datetime.now.year == vehicle.date_of_first_registration:
            cost = self.calculate_first_years_tax(vehicle)
        else:
            cost = self.calculate_tax_for_subsequent_years(vehicle)
        return cost

    def calculate_first_years_tax(self, vehicle):
        ...
        if vehicle.fuel_type is "PETROL":
            ...
            raise Exception("The supplied fuel type was not recognized: ", vehicle.fuel_type )
        return cost

    def calculate_tax_for_subsequent_years(self, vehicle):
        if vehicle.fuel_type is "PETROL":
            cost = 140
        elif vehicle.fuel_type is "DIESEL":
            cost = 140
